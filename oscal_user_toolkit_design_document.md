...
### 10.28 Save/load round-trip tests for SSP, AP, AR, and POA&M (`tests/test_roundtrip.py`)

Follows directly from §10.26's own "suggested next" list: `models.py`'s `build_oscal_ssp()`/`parse_ssp_file()` and their AP/AR/POA&M equivalents had zero test coverage — the single largest untested surface in the one file that's actually easy to test. Coverage of `models.py` went from 15% to 48% (measured with `coverage.py`) with this one file.

**Every fixture was built empirically, not guessed from the schema.** For each document type, an internal working dict (`empty_ssp()` etc.) was populated with one representative entry per list field, run through `build_oscal_*()` then `parse_*_file()`, and the *actual* printed output was read before writing any assertion — so what's asserted is what the code actually does today, not what it "should" do. This mattered: it surfaced two real, previously-undiscovered bugs in the process, neither fixed yet (deliberately — flagged to the designer rather than silently patched mid-test-writing):

- **`build_oscal_ar()` never writes an AR observation's `assessed_by` value to a prop**, even though `build_oscal_poam()` does exactly that for the same internal field name, and `parse_ar_file()` reads it back via the same shared `_parse_oscal_observation()` helper both document types use. In practice this is low-impact — `ar_tab.py`'s own UI never collects or displays `assessed_by` for AR observations at all, so the field is always empty in normal use — but a hand-crafted or another-tool's AR file with that prop set would silently lose it on the first save through this app. `test_ar_round_trip()` documents this as *current* behaviour (asserts the value comes back `""`, not the input value), specifically so a future fix changes a passing assertion rather than silently "just working."
- **README.md and this document's own feature list claim AR risks carry "CIA impact characterizations... stored as OSCAL facets"** — checked directly: `build_oscal_ar()`'s risk-building loop has no CIA handling at all, `parse_ar_file()`'s risk-parsing loop doesn't read any back, and `ar_tab.py` never collects `cia_c`/`cia_i`/`cia_a` from the user. That feature exists only for POA&M risks (`build_oscal_poam()`, confirmed present and round-tripping correctly in `test_poam_round_trip()`). The documentation claim is inaccurate as written — not yet corrected, flagged for a follow-up doc fix.

**9 new tests**: full round-trips for SSP (with roles/parties/responsible-parties/components/information-types/VLANs/control-implementations/users/inventory-items), AP (both `reviewed_controls_all` branches, plus an `on-date` and an `at-frequency` task), AR, and POA&M (including the CIA-facet round-trip that AR was missing at the time — see §10.29 — and `import_ssp`'s absolute-to-relative path conversion via `save_path`), plus an empty-SSP safety check and two `oscal_version` required-argument checks. Both bugs found here were fixed in the very next pass — see §10.29.

### 10.29 Fixed both §10.28 bugs: AR now writes `assessed_by`, and AR risks get real CIA characterization support

Per direct instruction, and picking the "add the missing feature" option over "just fix the docs" for the second bug (rationale: AR is where a risk is first identified during an assessment, so capturing its CIA impact rating there — not only after export to POA&M — fits the actual workflow better than walking back a stated feature).

**`assessed_by` fix.** `build_oscal_ar()`'s observation-building loop now writes the same `{"name": "assessed-by", "value": ..., "ns": "https://oscal-user-toolkit/ns/poam"}` prop `build_oscal_poam()` already wrote — one `if o.get("assessed_by"): entry["props"] = [...]` block, copied verbatim from the POA&M side. No `parse_ar_file()` change needed — it already read this back via the shared `_parse_oscal_observation()` helper, so the bug really was one-directional (write-side only).

**CIA characterization support added to AR risks, matching POA&M's exactly.** Three pieces, all mirroring `poam_tab.py`/`build_oscal_poam()`/`parse_poam_file()`'s existing, working implementation rather than inventing a new encoding:
- `ar_tab.py`'s `_risk_dialog()` gained the same three `Confidentiality Impact`/`Integrity Impact`/`Availability Impact` combo fields (`CIA_IMPACTS = ["", "high", "moderate", "low", "very-low", "not-applicable"]`) as `poam_tab.py`'s, and now returns `cia_c`/`cia_i`/`cia_a` in the result dict. Existing rows below them (Description, Impact Statement, Remediations, Remarks) shifted down 3 grid rows to make room — a purely mechanical renumbering, not a redesign.
- `build_oscal_ar()`'s risk loop gained the identical `characterizations`-facet-building block `build_oscal_poam()` already had (same `_CIA_SYSTEM` namespace string, same `origin.actors[0].type = "tool"` OSCAL-1.2.2-required-field pattern), only emitted `if any(cia.values())` so a risk with no impact rating set doesn't grow an empty `characterizations` array.
- `parse_ar_file()`'s risk loop gained the identical facet-reading block `parse_poam_file()` already had, extracting `cia_c`/`cia_i`/`cia_a` back out.
- **No change needed** to `ar_tab.py`'s `_generate_poam()` (the "Generate POA&M from Findings" button) — it already does `poam_tab._risks.append(dict(ar_risk_by_uuid[risk_uuid]))`, a shallow copy of the *entire* risk dict on the documented assumption that "POA&M risks use the same internal format" as AR's. Since AR's risk dict shape now actually includes `cia_c`/`cia_i`/`cia_a`, that copy carries the impact rating through automatically — the assumption the existing comment stated is now actually true, rather than accidentally true for every field except this one.

**Verified functionally**, not just syntax-checked: simulated `_risk_dialog()`'s own OK button being clicked (monkeypatching `wait_window` to invoke the OK button's callback directly rather than blocking, then reading the returned dict) and confirmed `cia_c`/`cia_i`/`cia_a` come back correctly. Updated `test_ar_round_trip()` in `tests/test_roundtrip.py` to assert the *fixed* behaviour (`assessed_by` now equals the input value, not `""`; risks round-trip their CIA fields) instead of documenting the bug — all 35 tests still pass, Ruff clean, full app construction OK.

### 10.30 GitHub CodeQL code scanning enabled

The designer enabled GitHub's default CodeQL setup via the repo's Security tab (no repo file to add — this is a GitHub-side setting, not a workflow this project's own files control). Complements the existing Ruff/pytest CI (§10.26): Ruff catches style/correctness issues, CodeQL does deeper security-focused taint analysis — relevant given the app's whole job is parsing JSON files a user picked off disk, which `SECURE_CODING.md` already treats as untrusted input. Free, since this repo is public.

### 10.31 Fixed the `COMPONENT_TYPES`/`SSP_COMPONENT_TYPES` `process-procedure` gap (`todo.md` §6)

Confirmed directly against `oscal_component_schema.json`'s `defined-component.type` enum (same check already done for §11's note below, just acted on this time): the schema's own term is the single value `"process-procedure"` — not `"process"` and `"procedure"` as two separate values. Both `component_tab.py`'s `COMPONENT_TYPES` and `ssp_tab.py`'s `SSP_COMPONENT_TYPES` had independently split it into those two non-schema terms; fixed both to offer the single correct term instead. Checked first that no real data (library or system components) used `"process"`/`"procedure"` as a saved type — none did, so no migration was needed.

`ssp_tab.py`'s own comment on `SSP_COMPONENT_TYPES` claimed `"process-procedure"` was "not valid in OSCAL 1.2.2" and that `"process"`/`"procedure"` had been added to replace it — checked directly against the schema and found that claim backwards; corrected the comment alongside the fix. Also updated the `.drawio` export's `TYPE_STYLES` colour map (`ssp_tab.py`), which had separate (but identical) fill-colour entries for `"process"` and `"procedure"` — collapsed to one `"process-procedure"` entry, since the two-value split there would otherwise have silently stopped matching once the dropdown itself no longer produced either value.

**Removed a dead duplicate** found while checking every reference: `models.py` had its own separate, unused `COMPONENT_TYPES` constant with the identical `"process"`/`"procedure"` split — never imported or referenced anywhere in the codebase. Deleted rather than fixed, since keeping an unused constant in sync with the two real ones is exactly how this kind of drift happens in the first place.
...
### 10.33 Extracted the OSCAL version scanner and ttk styling out of `app.py`

Prompted by an external suggestion (a "split `app.py` into a package" recommendation) — evaluated rather than applied wholesale: `app.py` at 1,857 lines is genuinely the biggest concern-mixing file in the codebase, but most of its bulk (the toolbar builder, notebook/tab wiring, theme manager) is deeply threaded through `self._catalog`, `self._profile`, and every tab instance's callbacks — splitting those into a separate class would mean either passing `app` around everywhere or duplicating state, not actual simplification. Two pieces genuinely had no such coupling and were worth pulling out:

- **`_scan_oscal_versions()` → `settings.scan_oscal_versions(oscal_dir)`**: was already a near-pure function (scans a folder for `oscal-*.zip` files, returns sorted version labels) that only touched `self` to stash its side-table of `{version: path}` — moved to `settings.py` (which already owns "configured paths" concepts — Library folder, Systems folder) as a proper pure function returning `(labels, paths)` as a tuple instead of a side effect. `app.py`'s `__init__` now does `self._oscal_versions, self._oscal_version_paths = settings.scan_oscal_versions(...)`. `get_oscal_versions()`/`get_oscal_version_paths()` stayed on `OSCALApp` — they're one-line dict/list accessors serving as the app's callback API for other tabs, not logic worth relocating.
- **`_style_ttk()` → `tab_utils.style_ttk(root, colors)`**: moved to `tab_utils.py` (already the home for other shared, non-tab-specific UI helpers — `attach_tooltip()`, `make_collapsible()`) with `colors` taken as an explicit parameter instead of reading the module-level `COLORS` global directly, so the function has no import-time dependency on `app.py` at all. Both of `app.py`'s call sites (startup, and `set_theme()`'s live re-style on toggle) now call `style_ttk(self, COLORS)`.

New `tests/test_settings.py` covers `scan_oscal_versions()` (missing directory, empty directory, non-zip/non-OSCAL files ignored, newest-first sort) — exactly the payoff of extracting it: it's now a plain function anyone can unit test without constructing a `Tk` window. `_style_ttk`'s replacement isn't separately unit tested (still tkinter/`ttk.Style` code — same rationale as every other GUI function in this codebase), but the theme-toggle path exercises it functionally.

**Verified functionally**: constructed the full app, confirmed `_oscal_versions`/`_oscal_version_paths` are populated identically to before, called `get_oscal_versions()`, and toggled the theme twice (light → dark) to exercise `style_ttk()`'s second call site — no errors, `app.py` down to 1,766 lines.

### 10.34 Fixed a real memory leak, and a missing scroll guard, found by an external code review

An external code review flagged four findings. Checked each directly rather than trusting the description at face value — one turned out to be a real, app-wide leak substantially bigger than the review's own framing suggested; the other three were confirmed exactly as described:

**The leak (review's finding #1, described only vaguely as "callback closures... unused variables"; the real bug was found by independently investigating rather than by that description).** Every scrollable tab's `_build_canvas()` called `canvas.bind_all("<MouseWheel>", self._on_mousewheel)`, and `theme_refresh()` (the dark/light toggle) destroys and rebuilds every tab's widgets — re-running that same call — each time the theme changes. Confirmed with a `weakref` test before touching any code: `bind_all()` never releases the Tcl command behind a previous binding when called again, so 20 rebind cycles using the raw call left all 20 old objects permanently unreachable to Python's garbage collector — each theme toggle leaks one entire previous tab instance (every loaded SSP, component, capability it held, all of it), for the life of the process.

**First fix attempt had its own bug, caught before landing.** Tracking `bind_all()`'s returned funcid and calling `deletecommand()` on the previous one before creating a new one *does* fix the leak (same weakref test: 20 cycles → 1 survivor) — but raises a `TclError` later, whenever the widget that originally registered the now-manually-deleted command is itself destroyed, because tkinter's own per-widget cleanup tries to delete the same command a second time. Caught this via the exact same test harness (calling `app.destroy()` afterward) before considering the fix done.

**The actual fix — new `tab_utils.bind_mousewheel(widget, handler)`**: calls `widget.bind_all("<MouseWheel>", ...)` exactly **once** for the life of the process, wiring a single permanent dispatcher function to the Tcl event. Every subsequent call just reassigns a module-level "which handler is currently active" variable — a plain Python assignment, not new Tcl-level state — so the previous handler (and whatever tab it belonged to) has no remaining reference from the Tcl layer and is collected normally, and no widget's own command bookkeeping is ever touched by code outside itself. All 9 real `bind_all("<MouseWheel>", ...)` call sites (`ap_tab.py`, `ar_tab.py`, `capability_tab.py`, `catalog_tab.py`, `component_tab.py`, `dashboard_tab.py`, `poam_tab.py`, `ssp_tab.py`, `workspace_tab.py`) now go through it.

**Findings #2 and #4 were the same bug reported twice** (missing `is_tab_active()` guard, at two different severities): `dashboard_tab.py` and `workspace_tab.py` were the only two scrollable tabs whose mousewheel handler didn't check `is_tab_active()` before scrolling — every other tab already did. Both now follow the same `try: if is_tab_active(self): ... except tk.TclError: pass` pattern already used everywhere else (`catalog_tab.py` was deliberately left on its own, different guard — a mouse-position check via `winfo_containing()` rather than `is_tab_active()` — since that's a legitimate alternate strategy, not a bug, and untouched by this review).

**Finding #3 confirmed**: `dashboard_tab.py`'s `profile_title` was assigned, conditionally reassigned, then never read — `profile_info` (a different variable) is what's actually displayed. Ruff's lint didn't catch this because `profile_title` is technically "read" by its own `if not profile_title:` check, enough to satisfy a simple unused-variable check even though the whole construct is dead. Removed.

**Verified functionally, twice over**: `tests/test_tab_utils.py` (3 new tests) proves the leak fix with the same weakref methodology used to find and confirm it — not just that the code "looks right" — plus confirms the dispatcher fires the most-recently-bound handler and calls `bind_all()` at most once regardless of how many tabs rebind. Separately, exercised the real app directly: captured a weakref to the Dashboard tab's canvas across 6 real theme toggles via `OSCALApp.set_theme()` and confirmed only the current canvas remains reachable; toggled the notebook's selected tab and confirmed `is_tab_active()` correctly returns `True`/`False` for the Dashboard tab in each case.
...
**A note on the `type` field**: the app's `COMPONENT_TYPES`/`SSP_COMPONENT_TYPES` lists previously split the schema's single `process-procedure` enum value into two separate dropdown options, `"process"` and `"procedure"` — neither was the literal schema term (though both, and `process-procedure`, were schema-valid since `type` also accepts any free-form string). ✅ **Fixed** — see §10.31.
...
**`COMPONENT_TYPES`'s `process-procedure` split** — fixed in §10.31.

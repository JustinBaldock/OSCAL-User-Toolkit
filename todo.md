...
## Backed by a user story or usability review

### 1. Capability version / revision history
...
Backed by [user_stories.md](user_stories.md) US-14, which explicitly names this as a known asymmetry: "Per-component version history... is now implemented for components... Not yet done: the same for capabilities."

### 2. Remaining usability gaps
...
## Ideas not yet linked to a user story

Checked directly against [user_stories.md](user_stories.md): nothing in it currently asks for these. They were written speculatively — real OSCAL capability gaps, but not yet tied to a validated need. Kept here rather than deleted in case a real need shows up later, but they shouldn't be picked up ahead of anything in the section above without first writing (or finding) the user story that justifies them.

### 3. Profile Editor

#### Purpose
Allow users to create and edit OSCAL 1.2.2 `profile` documents within the application. Currently the app can load a profile as a filter over the catalog, but profiles can only be created or modified by hand-editing JSON.

US-1 only asks to *load* a profile as a filter over the catalog (already implemented) — it doesn't ask to *create or edit* one in-app. No other story mentions profile authoring either.
...
### 4. Standalone Component Definition document type

The Library system (see design document §10.13–§10.20) already covers most of what a "reusable, shareable component library separate from any one SSP" originally needed — a shared folder, an import path into any SSP, dedicated Library-mode editors. US-2's and US-14's acceptance criteria for this are already fully met and marked implemented. What's listed below is extra, beyond what either story actually asked for:
...
### 5. Multi-Catalog Support — extend beyond the Library editors
...
US-1's own acceptance criteria explicitly accepts "only one catalog and one profile active at a time" as the implemented, satisfactory status — so extending this further isn't asked for by that story; it's speculative beyond it.
...
If a Profile Editor (§3 above) is ever built, it should support adding multiple `imports` entries — each with its own catalog href and control selection. The merge strategy (`as-is` vs `combine`) should be configurable when more than one import is present.
...
Once multi-catalog support is implemented, create an example overlay profile in `systems/example-03-nist/`:
...
US-3 mentions "network architecture, data flow" only as sections that must be readable in the SSP export — it doesn't ask for diagram generation from them.
...
|---|---|---|---|
| Capability version/revision history | Medium | Low | Backed by US-14's own asymmetry note — see §1 |
| Batch operations / multi-select | Low | High | No existing Treeview supports multi-select — see §2 |
| Undo/redo | Low | High | Needs a real change-tracking layer — see §2 |
| Profile Editor | Low | High | Not linked to a user story yet — see §3 |
| Standalone Component Definition document type | Low | Medium | Not linked to a user story yet — see §4 |
| Multi-Catalog Support — extend to System Overview editors | Low | Medium | Not linked to a user story yet — see §5 |
| Data Flow Diagram Export | Low | Medium | Not linked to a user story yet — see §6 |

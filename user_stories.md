...
- **Status: implemented — as a separate tab, not the existing Dashboard.** Rather than teach `dashboard_tab.py` to hold multiple networks (it stays exactly as it was: single-document, reading the live editor tabs), a new **🌐 All Systems** tab (`all_systems_tab.py`) reads directly from disk instead: it scans a configured **Systems folder** (`settings.py`, one subfolder per system, each with its own workspace manifest) and shows a per-system row (compliance %, open risks, POA&M overdue, last assessed) plus an organisation-wide rollup. This resolves the acceptance criteria above without needing "add this network's documents to the Dashboard" as its own action — a system is included automatically just by having a subfolder in the Systems folder, and removed by taking it out. See design document §10.19.

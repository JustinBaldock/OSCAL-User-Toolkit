...
- **Status: partially implemented — the import half is done, the consuming half is not.** Component Editor and Capability Editor now have a "📚 Import from Library" button (`_import_from_library()` in `component_tab.py`/`capability_tab.py`) that copies a file from the configured Library folder (`app.py` "📚 Library Folder" button, persisted via `settings.py`) into `<current system folder>/components/` or `.../capabilities/`, skipping any file that's already been imported so local edits aren't clobbered, then loads the copy for editing. The "current system folder" is the folder containing the active workspace manifest (`app.get_system_folder()`) — a workspace must be open/saved first. What's **not yet built**: the SSP Editor (Section 8/9), Assessment Plan, and POA&M editors still don't read a system's folder at all — `ssp_tab.py` still uses its older, separate `_import_components_from_files`/`_import_capability_into_ssp` mechanism straight into its own in-memory list, unrelated to the new Library→system-folder pipeline. Connecting those is the next stage.

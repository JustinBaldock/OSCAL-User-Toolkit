...
- **Status: implemented — mechanism changed.** Opening/clearing a catalog/profile now happens in the Data Sources tab (`data_sources_tab.py`), which browses the configured Library's `catalogs/`/`profiles/` subfolders and can still browse elsewhere for anything outside the library — the toolbar's old "Open Catalog"/"Open Profile"/"Clear Profile" buttons were removed. Only one catalog and one profile are active at a time, same as before (see `todo.md` §5 for the still-open multi-catalog question, and US-15 below for the Library-editor multi-catalog support that is implemented).
...
**Acceptance criteria:**
- Can import one or more components from a shared Library into the current system's own folder, as independent editable copies.
- Can import a capability (and its member components) from the Library into the current system's folder the same way.
- Once imported, the component/capability becomes an independent, editable copy scoped to this system — editing it does not alter the Library's source file, and re-importing the same source doesn't silently overwrite an already-imported local copy.
...
- **Status: implemented.** A new **Organisation** tab (between Data and System Overview) has dedicated "⚙ Library Components" and "🔗 Library Capabilities" editors — second instances of `ComponentTab`/`CapabilityTab` with a new `library_mode=True` flag (see design document §10.20) that locks them to `library/components/`/`library/capabilities/`: no Open File(s)/Open Folder/Import from Library, auto-loads every file in the Library on open and via "🔄 Refresh from Library," and saves write back with no location prompt (a new `_component_paths`/`_capability_paths` map remembers where each item came from; a new item auto-names into the Library). A teal hint banner explains the purpose and that edits don't auto-propagate to systems that already imported a copy. A "📥 Add File to Library" button is the one controlled way to bring an external file in. Since these are separate tab instances entirely, there's no ambiguity about which one you're editing — the tab itself answers that, rather than needing an in-editor indicator. Per-component version history using OSCAL's native `metadata.revisions[]` is now also **implemented** for components (design document §10.21, `todo.md` §1), so a normal user can see whether the Library's copy of a component has been updated since their system last imported it. **Not yet done**: the same for capabilities, which still use shared tab-level version state with no revision history.

The Library itself now ships with 124 example components covering every OSCAL component type and 11 example capabilities (see design document §11) — populated specifically to give an Organisation User a realistic, comprehensive starting point to maintain and extend, rather than a handful of illustrative examples.

### US-15: Edit Library components to support multiple catalogues, not just the ISM

**As an** Organisation User,
**I want to** edit a Library component so its control responses can reference more than one catalogue (e.g. the ISM and NIST SP 800-53),
**so that** a single shared component (e.g. "AWS" or "SSH") can serve every system that imports it, regardless of which regulatory framework that system is assessed against — without needing a separate near-duplicate copy of the component per framework.

**Acceptance criteria:**
- When a profile that imports more than one catalogue is loaded, the Library Component and Capability Editors' control lists show controls from every loaded catalogue, not just one.
- A component/capability can hold responses against controls from different catalogues at once, and each response is saved against the catalogue it actually came from.
- Saving still produces schema-valid OSCAL output — a component with multi-catalogue responses is not a special case the schema rejects.
- **Status: implemented.** `get_resolver()` (`app.py`) is wired into the Library Component and Capability Editors only (not the System Overview editors — see the note on scope below): when the loaded profile's `imports[]` pulls in more than one catalogue, `ComponentTab._get_profile_controls()`/`CapabilityTab._get_controls()` combine controls from every loaded catalogue, tagging each row `[source_filename]` so it's clear which catalogue a control came from. A new `ctrl_response_sources` dict records which catalogue each individual response's control came from; `build_component_oscal_entry()`/`CapabilityTab._build_oscal_document()` group responses by that source and emit one `control-implementations` block per catalogue — confirmed directly against `oscal_component_schema.json` that this is schema-native (the array has no maximum, and each block carries its own required `source`), not a workaround. `library/components/service_ssh.json`, `policy_privacy.json`, `service_learning_management_system.json`, `software_drupal.json`, `software_django.json`, and `service_aws.json` were all converted to this pattern (ISM + NIST SP 800-53 rev5 responses on the same component) as real examples. See design document §10.24 for the full design, including why System Overview's own editors were deliberately left out of scope (they stay tied to whatever single catalogue/profile is loaded for one system) and how a control-ID collision across catalogues is handled.
...
- **Status: implemented — as a separate tab, not the existing Dashboard.** Rather than teach `dashboard_tab.py` to hold multiple networks (it stays exactly as it was: single-document, reading the live editor tabs), a new **🌐 All Systems** tab (`all_systems_tab.py`) reads directly from disk instead: it scans a configured **Systems folder** (`settings.py`, one subfolder per system, each with its own workspace manifest) and shows a per-system row (controls required, controls applied, compliance %, open risks, POA&M overdue, last assessed) plus an organisation-wide rollup. **Controls Required**/**Controls Applied** were added later (design document §10.36) — the profile's selection size versus how many controls actually have a recorded response in the SSP. This resolves the acceptance criteria above without needing "add this network's documents to the Dashboard" as its own action — a system is included automatically just by having a subfolder in the Systems folder, and removed by taking it out. See design document §10.19.
...
- **Status: partially implemented.** `ap_tab.py` has "🔄 Refresh from SSP" to pull components from the referenced SSP's file (Section 2a), plus a new read-only Capabilities pane (Section 2b) populated from the same parsed SSP's `capabilities_used` list — name only, since OSCAL's SSP schema has no native capabilities structure to recover member components from outside the toolkit's own state (see design document §10.15). "📋 Load IDs from profile" populates control IDs from the app's currently loaded profile (now set via the Data Sources tab), and still draws from whichever profile happens to be currently active rather than specifically the control set recorded against the referenced SSP itself — worth checking these two sources always agree before treating control coverage as complete.

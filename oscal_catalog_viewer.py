...
def get_prop(props: list, name: str, default="—") -> str:
    """Return the first value whose 'name' matches, else default."""
    for p in props:
        if p.get("name") == name:
...
def get_all_props(props: list, name: str) -> list[str]:
    """Return all values whose 'name' matches."""
    return [p.get("value", "") for p in props if p.get("name") == name]


def get_statement(parts: list) -> str:
    """Extract the prose from the 'statement' part."""
    for part in parts:
...
def collect_controls(obj: dict, parent_titles: list[str] | None = None) -> list[dict]:
    """Recursively collect all controls with breadcrumb path."""
...
def load_catalog(filepath: str) -> dict:
    """Load and parse an OSCAL catalog JSON file."""
...
class OSCALViewer(tk.Tk):
    ...
    def _style_ttk(self):
        ...
        s.configure("TCombobox",
                     fieldbackground=self.HEADER_BG,
                     background=self.HEADER_BG,
                     foreground=self.TEXT,
    ...
    def _build_body(self):
        ...
        # Mouse-wheel scroll on detail pane
    ...
    def _populate_tree(self, controls: list[dict]):
    ...
    def _show_placeholder(self):
        for w in self._detail_frame.winfo_children():
            w.destroy()
        msg = "Select a control from the list to view details." if self._catalog \
              else "Open an OSCAL catalog file to get started."
        tk.Label(
    ...
    def _row(self, parent, label: str, value: str,
             value_color: str | None = None, italic: bool = False):
    ...
    def _on_mousewheel(self, event):
        # Only scroll when pointer is over the detail canvas

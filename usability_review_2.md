...
**Follow-up #2**: the Data Sources tab's "Clear Profile" button was reported as still showing the wrong text colour after the above fix — turned out to already be corrected in code (`bg=SECONDARY_BG, fg=BUTTON_TEXT`); likely a stale running instance. While checking, also found two dynamically-built button loops (`ap_tab.py`'s Objective/Task Add/Edit/Remove row, `ar_tab.py`'s Add/Edit/Remove rows) that still hard-coded `bg=C["HEADER_BG"]` for their Edit/Remove buttons — missed by the earlier sweep because their colours are chosen inside a loop rather than written literally at each `tk.Button()` call. Fixed both to `SECONDARY_BG`.

**Follow-up #3**: separately, checked every `tk.Button()` in the app for bold font — found 94 instances using `font=("Helvetica", N, "bold")`, inconsistent with the rest (most buttons were already plain weight). Removed `"bold"` from all of them, including the two loop-based buttons above where it was applied conditionally (`"bold" if bg == C["BLUE_BG"] else "normal"`).
